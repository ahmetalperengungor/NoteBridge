document.addEventListener('DOMContentLoaded', async function () {
  ...
  var posts = await makeHttpRequest('/saves/posts', 'GET')
  ...
  var goToLikedBtn = document.getElementById('go-to-liked-btn')

  function renderPosts() {
    postContainer.innerHTML = ''

    if (posts.length === 0) {
      postContainer.innerHTML = '<p>No saved posts available. Please save some posts to view them here.</p>'
      return
    }

    filteredPosts.forEach(function (post, index) {
      if (post.description && post.imageUrls && post.imageUrls.length > 0 && post.title && post.type && post.username) {
        ...
        postElement.innerHTML = `
                    ...
                    <div class="content">
                        ...
                        ${post.type === 1 ? `<p><strong>Location:</strong> ${post.location}</p>` : ''}
                    </div>
                `
  ...
  function openModal(post) {
    ...
    modalContent.innerHTML = `<p>${post.description}</p>${
      post.type === 3 ? `<p><strong>Location:</strong> ${post.location}</p>` : ''
    }`
  ...
  goToLikedBtn.addEventListener('click', function () {
